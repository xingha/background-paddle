...
class Refiner(nn.Layer):
    ...
    def forward(self,
        ...
        src_bgr = paddle.cat([src, bgr], dim=1)
        ...
        if self.mode != 'full':
            ...
            if idx[0].size(0) > 0:
                x = paddle.cat([hid, pha, fgr], dim=1)
                ...
                x = self.conv1(paddle.cat([x, y], dim=1))
                x = self.bn1(x)
                x = self.relu(x)
                x = self.conv2(x)
                x = self.bn2(x)
                ...
                x = self.conv3(paddle.cat([x, y], dim=1))
                x = self.bn3(x)
                x = self.relu(x)
                x = self.conv4(x)
                
                out = paddle.cat([pha, fgr], dim=1)
            ...
            pha = x[:, :1]
            fgr = x[:, 1:]
            ref = paddle.ones((src.size(0), 1, H_quat, W_quat), device=src.device, dtype=src.dtype)
    ...
    def crop_patch(self,
        ...
        if self.patch_crop_method == 'unfold':
            ...
            boxes = paddle.stack([b, x1, y1, x2, y2], dim=1)
            return paddle.fluid.layers.roi_align(x, boxes, size + 2 * padding, sampling_ratio=1)
        else:
            # Use gather. Crops out patches pixel by pixel.
            idx_pix = self.compute_pixel_indices(x, idx, size, padding)
            pat = paddle.gather(x.view(-1), 0, idx_pix.view(-1))
